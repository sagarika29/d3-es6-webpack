...
// const o2p = data[data.map(item => item.name).indexOf('O2P')].values.filter(link => link.source.id !== null && link.target.id !== null); // Null check in data of relations
// //const categories = [...new Set(organizations.map(o => o.category))];
// let nodes = [...organizations,...projects,...cities];
// console.log(nodes);
// let links = o2p.map(o2p => Object.assign({}, {
//     data: o2p,
...
//     const activeNode = d;
//     const activeNodeLinks = o2p.reduce((links, link) => {
...
//     svg.selectAll('.node > rect').attr('stroke-opacity', d => activeNodeLinks.indexOf(d.id) > -1 ? 1 : 0.2).attr('fill-opacity', d => activeNodeLinks.indexOf(d.id) > -1 ? 1 : 0.5);
...
//     svg.selectAll('.node > rect').attr('stroke-opacity', 2).attr('fill-opacity', 0.5);
...
// // let xDiff = 500;
// // let yDiff = 500;
// // let pointData = [[A+xDiff, 0+yDiff], [A/2+xDiff, A*_s32+yDiff], [-A/2+xDiff, A*_s32+yDiff], [-A+xDiff, 0+yDiff],
// // [-A/2+xDiff, -A*_s32+yDiff], [A/2+xDiff, -A*_s32+yDiff]];


// node.each(function(d) {
//     if (d.type == "organization") {
//       d3.select(this).append("circle").attr('fill', d => color(d.category)).attr('r', radius).attr('cx', 0).attr('cy', 0).attr('stroke', 'black').attr('stroke-width', 2).attr('fill-opacity', 0.5).attr('stroke-opacity', 0.2);
//     } else if(d.type == "project") {
//       d3.select(this).append('rect').attr('fill', d => color(d.color)).attr("x", -(120/2)).attr("y", -(52/2)).attr("width", 120).attr("height", 52).attr('stroke', 'black').attr('stroke-width', 2).attr('fill-opacity', 0.5).attr('stroke-opacity', 0.2);
//     } 
//     // else if(d.type == "city"){
//     //   d3.selectAll("path.area").data([pointData]).enter().append("path").attr("d", d3.svg.line());
//     // }
//   });
//   node.append('text').text(d => d.abbreviation).attr('text-anchor', 'middle').attr('font-size', '11px');
...
// };
// const ticked = () => {
//     link.attr('x1', d => d.source.x - offset(d.source, d.target, radius).x).
...
//     d3.select(elem[i]).select('text').attr('font-size', '12px').attr('font-weight', 'bold');
...
//     d3.select(elem[i]).select('text').attr('font-weight', 'normal').attr('font-size', '11px');
...
// legend.append('text').text(d => d).attr('x', 20).attr('dy', 10).attr('font-size', '11px');
